...
$stmt = $pdo->query("SELECT id, product_code, product_name FROM products ORDER BY product_name ASC");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($productId <= 0 || $qty <= 0) {
        ...
        try {
            ...
            // Insert ke stock_in
            $stmtIn = $pdo->prepare("
                INSERT INTO stock_in (product_id, qty, stock_before, stock_after, transaction_date, note)
                VALUES (?, ?, ?, ?, ?, ?)
            ");
            $stmtIn->execute([$productId, $qty, $stockBefore, $stockAfter, $transactionDate, $note]);
            ...
            $pdo->rollBack();
            $error = 'Gagal menambah stock in: ' . $e->getMessage();
        }
    }
}

$pageTitle = 'Add Stock In';
...
<div class="container-fluid px-4 py-3">
    <div class="d-flex align-items-center justify-content-between mb-4">
        <div>
            <h4 class="mb-1 text-primary"><i class="bi bi-box-arrow-in-down me-2"></i>Add Stock In</h4>
            <p class="text-muted small mb-0">Tambah transaksi barang masuk KMS Medical.</p>
        </div>
    </div>
            ...
            <form method="POST">
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="number" name="qty" class="form-control rounded-pill" min="1" placeholder="Masukkan Qty" required>
                    </div>
                ...
                <div class="d-flex justify-content-between align-items-center mt-4">
                    ...
                    <button type="submit" class="btn btn-success rounded-pill px-4">
                        <i class="bi bi-check-circle me-1"></i> Save Stock In
                    </button>
                </div>
            </form>
        </div>

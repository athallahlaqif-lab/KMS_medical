...
// Panggil file helper
require_once __DIR__ . '/do_helper.php';
...
$sj_number = generateDONumber($transactionId);

if (!$data) {
    die("Data Delivery Order tidak ditemukan.");
}

$items = getTransactionItems($pdo, $transactionId);

if (count($items) === 0) {
    die("Data produk untuk Delivery Order ini tidak ditemukan.");
}

// ------------------------------------------------------------------
// PEMETAAN DATA DATABASE
// ------------------------------------------------------------------
...
<head>
    ...
    <title>Delivery Order - <?= htmlspecialchars($sj_number) ?></title>
...
<?= htmlspecialchars($sj_number) ?>
    ...
    READY TO SHIP
           ...
           <!-- CUSTOMER SECTION -->
<div style="width:60%; margin-bottom:16px;">
...
<div class="watermark">
    ...
    <script>
        window.onload = function () {
            document.title = "Delivery Order - <?= $sj_number ?>";
        };
    </script>

...
// Ambil data stock out & filter
require_once 'stockout_data.php';
...
<div class="container-fluid px-4 py-3">
    ...
    <div class="d-flex align-items-center justify-content-between mb-3">
        <div>
            <h4 class="mb-1 text-danger"><i class="bi bi-box-arrow-up me-2"></i>Stock Out</h4>
            <p class="text-muted small mb-0">Kelola seluruh transaksi barang keluar pada sistem KMS Medical.</p>
        </div>
        <div class="d-flex gap-2">
            ...
            <!-- Tombol Add Stock Out -->
            <a href="create.php" class="btn btn-danger rounded-pill px-3 shadow-sm">
    ...
    <?php if (isset($_GET['success'])): ?>
        <div class="alert alert-success alert-dismissible fade show rounded-3 shadow-sm" role="alert">
            <i class="bi bi-check-circle-fill me-2"></i> Transaksi stock out berhasil ditambahkan!
                        ...
                        <?php if (empty($stockOut)): ?>
                            <tr>
                                <td colspan="9" class="text-center py-4 text-muted">
                                    <i class="bi bi-inbox fs-3 d-block mb-1"></i> Tidak ada data transaksi stock out.
                                </td>
                            </tr>
                        <?php else: ?>
                            <?php $no = 1; foreach ($stockOut as $row): ?>
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <span class="badge bg-danger rounded-pill px-3"><?= number_format((float)$row['qty']); ?></span>
                                    </td>
                                    <td class="text-center text-muted"><?= number_format((float)$row['stock_before']); ?></td>
                                    <td class="text-center fw-bold text-danger"><?= number_format((float)$row['stock_after']); ?></td>
